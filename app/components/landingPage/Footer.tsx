...
import flare from "@/public/assests/flare.png";
...
const Footer: React.FC = () => {
  ...
  return (
    <Box className="relative mt-12 bg-transparent text-white">
      <style>
        {`
          ...
          <Stack key={index} spacing={2}>
            ...
            {section.links.map((link, linkIndex) => (
              <Link
                key={linkIndex}
                href={link.href}
                className="text-black dark:text-white/80 text-base hover:text-purple-800 dark:hover:text-pink-700 hover:underline transition duration-300"
              >
                {link.text}
              </Link>
            ))}
      ...
      <Divider className="border border-white my-6" />
      <Grid
        className="grid grid-cols-1 md:grid-cols-2 items-center text-left mt-8 px-4 md:px-12 pb-10"
      >
        <Stack className="space-y-4 pointer-events-none whitespace-pre-wrap bg-gradient-to-r from-blue-700 via-pink-600 to-purple-600 bg-clip-text text-center font-semibold leading-none text-transparent  ">
          <Text className="text-5xl font-bold ">© 2024 Dayflow.io Technologies, Inc. All rights reserved.</Text>
        ...
        <HStack className="flex justify-center text-4xl md:justify-end space-x-6 mt-4 md:mt-0 text-purple-800 dark:text-purple-100">
